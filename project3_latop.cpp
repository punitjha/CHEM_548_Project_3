...
//****************************************************************************************************************
	...
	if (myfile.is_open())
	{
		...
		istringstream ee(str1);//isting is used to extract all the words from the line
		ee >> atoms; // no. of atoms is assigned here
		connect=arma::zeros(atoms,3);// the connectivity matrix is defined it has 3 rows as any carbon can be at the maximum be connect to 3 carbons
	...
	arma::mat Huckel2= arma::zeros(atoms,atoms); //the Huckel matrix for trans-polyacetylene as a semi-conductor but has the value of alph and beta assigned to it in electron-volts.
	...
	Huckel2.diag().fill(5.94);//filling the diagonal matrix elements with the value of 5.94eV using the fill function in the armadillo library
...
//****************************************************************************************************************************
	...
	arma::cx_mat eigenvectors222;// the matrix wich will store the eigenvectors
        fstream myfile22("bandgap1.txt", fstream::out|fstream::trunc);	
	for(double k=-pi; k<=pi; k+=0.33)
	{
		ddHuckel(0,0)=0;
...
//***************************************************************************************************
//applying the program to single walled carbon nano tube with four carbon atoms on the circumference*
//***************************************************************************************************
	int carbon=4;
	fstream myfile33("carbon.txt", fstream::out|fstream::trunc);
	arma::cx_mat DHuckel(carbon,carbon);
	arma::cx_mat first(carbon,carbon);
	arma::cx_mat second(carbon,carbon);
	arma::cx_mat third(carbon,carbon);
...
//	third.print();
	...
	{
		DHuckel=first*exp(-I*k)+second+third*exp(I*k);
